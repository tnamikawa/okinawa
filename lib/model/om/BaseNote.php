...
/**
 * Base class that represents a row from the 'note_mst' table.
 *
 * 
 *
...
abstract class BaseNote extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the font_family field.
	 * @var        int
	 */
	protected $font_family;

	/**
	 * The value for the font_size field.
	 * @var        int
	 */
	protected $font_size;

	/**
	 * The value for the write_date field.
	 * @var        string
	 */
	...
	/**
	 * Get the [font_family] column value.
	 * 
	 * @return     int
	 */
	...
	/**
	 * Get the [font_size] column value.
	 * 
	 * @return     int
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [write_date] column value.
	 * 
	 *
	...
	{
		if ($this->write_date === null) {
			return null;
		}


		if ($this->write_date === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->write_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->write_date, true), $x);
			}
		}
	...
	/**
	 * Set the value of [font_family] column.
	 * 
	 * @param      int $v new value
	 * @return     Note The current object (for fluent API support)
	 */
	...
	/**
	 * Set the value of [font_size] column.
	 * 
	 * @param      int $v new value
	 * @return     Note The current object (for fluent API support)
	 */
	...
	/**
	 * Sets the value of [write_date] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setWriteDate($v)
	{
		...
		if ( $this->write_date !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->write_date !== null && $tmpDt = new DateTime($this->write_date)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->write_date = ($dt ? $dt->format('Y-m-d H:i:s') : null);
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->font_family = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
			$this->font_size = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
			$this->write_date = ($row[$startcol + 6] !== null) ? (string) $row[$startcol + 6] : null;
			$this->resetModified();
			...
			return $startcol + 7; // 7 = NotePeer::NUM_COLUMNS - NotePeer::NUM_LAZY_LOAD_COLUMNS).
