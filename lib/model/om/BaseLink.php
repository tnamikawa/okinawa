...
/**
 * Base class that represents a row from the 'link_mst' table.
 *
 * 
 *
...
abstract class BaseLink extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the inserted_at field.
	 * @var        string
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [inserted_at] column value.
	 * 
	 *
	...
	public function getInsertedAt($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->inserted_at === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->inserted_at);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->inserted_at, true), $x);
			}
		}
	...
	/**
	 * Sets the value of [inserted_at] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setInsertedAt($v)
	{
		...
		if ( $this->inserted_at !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->inserted_at !== null && $tmpDt = new DateTime($this->inserted_at)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->inserted_at = ($dt ? $dt->format('Y-m-d H:i:s') : null);
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->inserted_at = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->resetModified();
			...
			return $startcol + 6; // 6 = LinkPeer::NUM_COLUMNS - LinkPeer::NUM_LAZY_LOAD_COLUMNS).
