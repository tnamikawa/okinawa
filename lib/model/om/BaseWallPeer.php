...
/**
 * Base static class for performing query and update operations on the 'wall_mst' table.
 *
 * 
 *
...
abstract class BaseWallPeer {
	...
	private static $fieldNames = array (
		...
		BasePeer::TYPE_STUDLYPHPNAME => array ('id', 'photoId', ),
		BasePeer::TYPE_COLNAME => array (self::ID, self::PHOTO_ID, ),
	...
	private static $fieldKeys = array (
		...
		BasePeer::TYPE_STUDLYPHPNAME => array ('id' => 0, 'photoId' => 1, ),
		BasePeer::TYPE_COLNAME => array (self::ID => 0, self::PHOTO_ID => 1, ),
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		{
		  call_user_func($sf_hook, 'BaseWallPeer', $criteria, $con);
		}
	 ...
	 */
	public static function addInstanceToPool(Wall $obj, $key = null)
	{
	...
	/**
	 * Method to invalidate the instance pool of all tables related to wall_mst
	...
	{
		return $withPrefix ? WallPeer::CLASS_DEFAULT : WallPeer::OM_CLASS;
	}

	/**
	 * Method perform an INSERT on the database, given a Wall or Criteria object.
	 *
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseWallPeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseWallPeer', $values, $con))
      {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseWallPeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseWallPeer', $values, $con, $pk);
    }
	...
	/**
	 * Method perform an UPDATE on the database, given a Wall or Criteria object.
	 *
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseWallPeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseWallPeer', $values, $con))
      {
        return $sf_hook_retval;
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseWallPeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseWallPeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the wall_mst table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	...
	/**
	 * Method perform a DELETE on the database, given a Wall or Criteria object OR a primary key value.
	 *
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BaseWallPeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
