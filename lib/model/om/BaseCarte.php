...
/**
 * Base class that represents a row from the 'carte_mst' table.
 *
 * 
 *
...
abstract class BaseCarte extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the ins_date field.
	 * @var        string
	 */
	protected $ins_date;
	...
	/**
	 * Get the [optionally formatted] temporal [ins_date] column value.
	 * 
	 *
	...
	public function getInsDate($format = 'Y-m-d')
	{
		...
		if ($this->ins_date === '0000-00-00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->ins_date);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->ins_date, true), $x);
			}
		}
	 ...
	 * @return     Carte The current object (for fluent API support)
	 */
	...
	/**
	 * Sets the value of [ins_date] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setInsDate($v)
	{
		...
		if ( $this->ins_date !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->ins_date !== null && $tmpDt = new DateTime($this->ins_date)) ? $tmpDt->format('Y-m-d') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->ins_date = ($dt ? $dt->format('Y-m-d') : null);
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->thumb_width = ($row[$startcol + 7] !== null) ? (int) $row[$startcol + 7] : null;
			$this->thumb_height = ($row[$startcol + 8] !== null) ? (int) $row[$startcol + 8] : null;
			$this->ins_date = ($row[$startcol + 9] !== null) ? (string) $row[$startcol + 9] : null;
			$this->resetModified();
			...
			return $startcol + 10; // 10 = CartePeer::NUM_COLUMNS - CartePeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = CartePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseCarte:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseCarte:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseCarte:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseCarte:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				$con->commit();
				CartePeer::addInstanceToPool($this);
				return $affectedRows;
			}
		} catch (PropelException $e) {
			$con->rollBack();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Carte (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Carte Clone of current object.
	 * @throws     PropelException
	 */
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseCarte:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseCarte::%s', $method));
	  }
