...
/**
 * Base static class for performing query and update operations on the 'link_mst' table.
 *
 * 
 *
...
abstract class BaseLinkPeer {
	...
	/** the column name for the INSERTED_AT field */
	...
	private static $fieldNames = array (
		...
		BasePeer::TYPE_STUDLYPHPNAME => array ('id', 'url', 'title', 'description', 'categoryId', 'insertedAt', ),
		BasePeer::TYPE_COLNAME => array (self::ID, self::URL, self::TITLE, self::DESCRIPTION, self::CATEGORY_ID, self::INSERTED_AT, ),
	...
	private static $fieldKeys = array (
		...
		BasePeer::TYPE_STUDLYPHPNAME => array ('id' => 0, 'url' => 1, 'title' => 2, 'description' => 3, 'categoryId' => 4, 'insertedAt' => 5, ),
		BasePeer::TYPE_COLNAME => array (self::ID => 0, self::URL => 1, self::TITLE => 2, self::DESCRIPTION => 3, self::CATEGORY_ID => 4, self::INSERTED_AT => 5, ),
	...
	/**
	 * Method to invalidate the instance pool of all tables related to link_mst
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseLinkPeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseLinkPeer', $values, $con))
      {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseLinkPeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseLinkPeer', $values, $con, $pk);
    }
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseLinkPeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseLinkPeer', $values, $con))
      {
        return $sf_hook_retval;
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseLinkPeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseLinkPeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the link_mst table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
