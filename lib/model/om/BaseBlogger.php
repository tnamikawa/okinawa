...
/**
 * Base class that represents a row from the 'blogpartsuser_mst' table.
 *
 * 
 *
...
abstract class BaseBlogger extends BaseObject  implements Persistent {
	...
	public function getLastAccess($format = 'Y-m-d')
	{
		...
		if ($this->last_access === '0000-00-00') {
	 ...
	 * @return     Blogger The current object (for fluent API support)
	 */
	...
	public function setLastAccess($v)
	{
		...
		if ( $this->last_access !== null || $dt !== null ) {
			...
			$currNorm = ($this->last_access !== null && $tmpDt = new DateTime($this->last_access)) ? $tmpDt->format('Y-m-d') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->last_access = ($dt ? $dt->format('Y-m-d') : null);
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->last_access = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->resetModified();
			...
			return $startcol + 3; // 3 = BloggerPeer::NUM_COLUMNS - BloggerPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = BloggerPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseBlogger:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseBlogger:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseBlogger:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseBlogger:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				$con->commit();
				BloggerPeer::addInstanceToPool($this);
				return $affectedRows;
			}
		} catch (PropelException $e) {
			$con->rollBack();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Blogger (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Blogger Clone of current object.
	 * @throws     PropelException
	 */
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseBlogger:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseBlogger::%s', $method));
	  }
