...
/**
 * Base class that represents a row from the 'top_photo' table.
 *
 * 
 *
...
abstract class BaseTop extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the link_color field.
	 * @var        string
	 */
	...
	/**
	 * Get the [link_color] column value.
	 * 
	 * @return     string
	 */
	 ...
	 * @return     Top The current object (for fluent API support)
	 */
	...
	/**
	 * Set the value of [link_color] column.
	 * 
	 * @param      string $v new value
	 * @return     Top The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->text_color = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->link_color = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->resetModified();
			...
			return $startcol + 4; // 4 = TopPeer::NUM_COLUMNS - TopPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = TopPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseTop:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseTop:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseTop:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseTop:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				$con->commit();
				TopPeer::addInstanceToPool($this);
				return $affectedRows;
			}
		} catch (PropelException $e) {
			$con->rollBack();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Top (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Top Clone of current object.
	 * @throws     PropelException
	 */
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseTop:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseTop::%s', $method));
	  }
