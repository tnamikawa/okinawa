...
/**
 * Base class that represents a row from the 'linkcategory_mst' table.
 *
 * 
 *
...
abstract class BaseLinkCategory extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the order_priority field.
	 * @var        int
	 */
	...
	/**
	 * Get the [order_priority] column value.
	 * 
	 * @return     int
	 */
	...
	/**
	 * Set the value of [order_priority] column.
	 * 
	 * @param      int $v new value
	 * @return     LinkCategory The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->order_priority = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->resetModified();
			...
			return $startcol + 3; // 3 = LinkCategoryPeer::NUM_COLUMNS - LinkCategoryPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseLinkCategory:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseLinkCategory:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseLinkCategory:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseLinkCategory:save:post') as $callable)
				{
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseLinkCategory:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseLinkCategory::%s', $method));
	  }
