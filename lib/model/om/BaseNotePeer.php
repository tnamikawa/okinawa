...
/**
 * Base static class for performing query and update operations on the 'note_mst' table.
 *
 * 
 *
...
abstract class BaseNotePeer {
	...
	/** the column name for the FONT_FAMILY field */
	const FONT_FAMILY = 'note_mst.FONT_FAMILY';

	/** the column name for the FONT_SIZE field */
	const FONT_SIZE = 'note_mst.FONT_SIZE';

	/** the column name for the WRITE_DATE field */
	...
	private static $fieldNames = array (
		...
		BasePeer::TYPE_STUDLYPHPNAME => array ('id', 'photoId', 'name', 'content', 'fontFamily', 'fontSize', 'writeDate', ),
		BasePeer::TYPE_COLNAME => array (self::ID, self::PHOTO_ID, self::NAME, self::CONTENT, self::FONT_FAMILY, self::FONT_SIZE, self::WRITE_DATE, ),
	...
	private static $fieldKeys = array (
		...
		BasePeer::TYPE_STUDLYPHPNAME => array ('id' => 0, 'photoId' => 1, 'name' => 2, 'content' => 3, 'fontFamily' => 4, 'fontSize' => 5, 'writeDate' => 6, ),
		BasePeer::TYPE_COLNAME => array (self::ID => 0, self::PHOTO_ID => 1, self::NAME => 2, self::CONTENT => 3, self::FONT_FAMILY => 4, self::FONT_SIZE => 5, self::WRITE_DATE => 6, ),
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		{
		  call_user_func($sf_hook, 'BaseNotePeer', $criteria, $con);
		}
	 ...
	 */
	public static function addInstanceToPool(Note $obj, $key = null)
	{
	...
	/**
	 * Method to invalidate the instance pool of all tables related to note_mst
	...
	/**
	 * Method perform an INSERT on the database, given a Note or Criteria object.
	 *
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseNotePeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseNotePeer', $values, $con))
      {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseNotePeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseNotePeer', $values, $con, $pk);
    }
	...
	/**
	 * Method perform an UPDATE on the database, given a Note or Criteria object.
	 *
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseNotePeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseNotePeer', $values, $con))
      {
        return $sf_hook_retval;
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseNotePeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseNotePeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the note_mst table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	...
	/**
	 * Method perform a DELETE on the database, given a Note or Criteria object OR a primary key value.
	 *
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BaseNotePeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
